...
namespace AniDroidv2.AniListObject.Media
{
    ...
    public class MediaActivity : BaseAniListObjectActivity<MediaPresenter>, IMediaView
    {
        ...
        private View CreateMediaUserDataView(AniList.Models.MediaModels.Media media)
        {
            ...
            if (media.Stats?.ScoreDistribution?.Count(x => x.Count > 0) >= 3 && !MediaStatus.NotYetReleased.Equals(media.Status))
            {
        ...
        private View CreateUserScoresView(IEnumerable<AniListScoreDistribution> scores)
        {
            ...
            var bars = scores.OrderBy(x => x.Score).Select(x => new BarEntry(x.Score, x.Count))
                .ToList();
        ...
        private View CreateUserListStatusView(IReadOnlyList<AniListStatusDistribution> statusDistribution)
        {
            ...
            var slices = statusDistribution.Select(x => new PieEntry(x.Count, x.Status.DisplayValue) { Data = x.Status.DisplayValue }).ToList();
            ...
            for (var i = 0; i < statusDistribution.Count; i++)
            {
                ...
                cell.FindViewById<TextView>(Resource.Id.ChartLegendCell_Count).Text = status.Count.ToTruncatedString();
