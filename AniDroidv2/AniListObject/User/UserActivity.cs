...
namespace AniDroidv2.AniListObject.User
{
    ...
    public class UserActivity : BaseAniListObjectActivity<UserPresenter>, IUserView
    {
        ...
        private View CreateUserDetailsView(AniList.Models.UserModels.User user)
        {
			...
			var imageView = retView.FindViewById<ImageView>(Resource.Id.User_Image);
			ImageLoader.LoadImage(imageView, user.Avatar.Large);
            imageView.Click += (sender, args) => MediaImageEnlarge.Create(this, user.Avatar.Large);
            ...
            var userAnimeListCount = user.Statistics.Anime.Count;
            if (userAnimeListCount > 0)
            {
                ...
                userAnimeView.TextTwo = $"{user.GetDurationString(user.Statistics.Anime.MinutesWatched * 60, 1)} watched";
            ...
            var userMangaListCount = user.Statistics.Manga.Count;
            if (userMangaListCount > 0)
            {
                ...
                userMangaView.TextTwo = $"{user.Statistics.Manga.ChaptersRead} chapters read";
        ...
        private View CreateUserStatsView(AniList.Models.UserModels.User user)
        {
            ...
            if (user.Statistics?.Anime?.Statuses?.Any(x => x.Count > 0) == true ||
                user.Statistics?.Manga?.Statuses?.Any(x => x.Count > 0) == true)
            {
                containerView.AddView(CreateStatusDistributionView(user.Statistics.Anime.Statuses,
                    user.Statistics.Manga.Statuses));
            }

            if (user.Statistics?.Anime?.Scores?.Any(x => x.Count > 0) == true ||
                user.Statistics?.Manga?.Scores?.Any(x => x.Count > 0) == true)
            {
                containerView.AddView(CreateScoreDistributionView(user.Statistics.Anime.Scores,
            ...
            IReadOnlyList<AniListStatusDistribution> mangaStatusDistributions)
        {
            ...
            var animeStatusEntries = animeStatusDistributions.Select(x => new BarEntry(x.Status.Index, x.Count, x.Status.DisplayValue)).ToList();
            var animeDataSet = new BarDataSet(animeStatusEntries, "Anime") { Color = colorList[0] };
            data.AddDataSet(animeDataSet);

            var mangaStatusEntries = mangaStatusDistributions.Select(x => new BarEntry(x.Status.Index, x.Count, x.Status.DisplayValue)).ToList();
            ...
            IReadOnlyList<AniListScoreDistribution> mangaScoreDistributions)
        {
            ...
            var animeEntries = Enumerable.Range(1, 10).Select(x => new BarEntry(x * 10, animeScoreDistributions.Any(y => y.Score == (x * 10)) ? animeScoreDistributions.First(y => y.Score == (x * 10)).Count : 0, (x * 10).ToString())).ToList();
            var animeDataSet = new BarDataSet(animeEntries, "Anime") { Color = colorList[4] };
            data.AddDataSet(animeDataSet);

            var mangaEntries = Enumerable.Range(1, 10).Select(x => new BarEntry(x * 10, mangaScoreDistributions.Any(y => y.Score == (x * 10)) ? mangaScoreDistributions.First(y => y.Score == (x * 10)).Count : 0, (x * 10).ToString())).ToList();
